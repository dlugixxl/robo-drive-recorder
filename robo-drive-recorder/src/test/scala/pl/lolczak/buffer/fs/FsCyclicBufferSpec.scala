 ...
 */
class FsCyclicBufferSpec extends FeatureSpec with Matchers with GivenWhenThen with BeforeAndAfter {
...
//      val emptyBuffer = new FsCyclicBuffer[SampleVo](location = bufferLocation, bufferSize = 100)
//
...
//  }
  ...
  feature("Retrieving single data element") {
    ...
    scenario("Retrieving data from buffer with one element") {
      ...
      val buffer = new FsCyclicBuffer[SampleVo](location = bufferLocation, bufferSize = 100)
  ...
  feature("Saving data into buffer") {
    scenario("Saving to large element") {
      ...
      val sampleElement = new SampleVo(SampleName, SampleAge, SampleMaturity)
      val buffer = new FsCyclicBuffer[SampleVo](location = bufferLocation, bufferSize = 20)
  ...
  feature("Buffer overlapping") {
    scenario("Guarantee of fixed size of buffer") {
      Given("empty buffer")
      val buffer = new FsCyclicBuffer[SampleVo](location = bufferLocation, bufferSize = 120)
      ...
      bufferLocation.length() should be <(2*120l)
    }

    scenario("Stores only not overwritten elements") {
      Given("overlapped bufer")
      val buffer = new FsCyclicBuffer[SampleVo](location = bufferLocation, bufferSize = 140)
      (1 to 100).foreach {
        i =>
          val sampleElement = new SampleVo(SampleName, i, SampleMaturity)
          buffer.put(sampleElement)
      ...
      elements.size shouldBe <(100) //2
      elements.takeRight(2) shouldBe List(new SampleVo(SampleName, 99, SampleMaturity), new SampleVo(SampleName, 100, SampleMaturity))
    }
  }
